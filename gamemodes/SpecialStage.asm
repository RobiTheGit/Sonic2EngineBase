...
; \------------------------------------------------------------------------/
	...
	bgt.s	+			; if yes, branch
	move.l	#Obj09,(MainCharacter).w ; load Obj09 (special stage Sonic)
	tst.w	(Player_mode).w		; is this a Sonic and Tails game?
	bne.s	++			; if not, branch
+	move.l	#Obj10,(Sidekick).w ; load Obj10 (special stage Tails)
+	move.l	#Obj5E,(SpecialStageHUD).w ; load Obj5E (special stage HUD)
	move.l	#Obj5F,(SpecialStageStartBanner).w ; load Obj5F (special stage banner)
	move.l	#Obj87,(SpecialStageNumberOfRings).w ; load Obj87 (special stage ring count)
...
SSTrack_Draw:
	...
	addi.b	#1,(SSTrack_anim_frame).w						; Increment current frame
	...
	addi.b	#1,(SpecialStage_CurrentSegment).w				; Increment current segment
...
Obj5F_Main:
	...
	move.w	#$48,y_pos(a0)
	move.b	#4,routine(a0)
	move.b	#$F,objoff_2E(a0)
	jmpto	(DisplaySprite).l, JmpTo_DisplaySprite
; ===========================================================================

loc_71B4:
	subi.b	#1,objoff_2E(a0)
    if ~~removeJmpTos
	bne.w	JmpTo_DisplaySprite
    else
...
; loc_736C:
Obj87_Init:
	move.b	#2,objoff_12(a0)		; => loc_7480
	...
	beq.s	+
	subi.b	#1,mainspr_childsprites(a0)
...
+
	bsr.w	SSSingleObjLoad
	bne.s	+	; rts
	move.l	#Obj87,(a1) ; load obj87
	move.b	#4,objoff_12(a1)		; => loc_753E
...
+
	...
	bne.s	-	; rts
	move.l	#Obj87,(a1) ; load obj87
	move.b	#6,objoff_12(a1)		; => loc_75DE
	move.l	#Obj5F_MapUnc_72D2,mappings(a1)
	move.w	#make_art_tile(ArtTile_ArtNem_SpecialHUD,2,0),art_tile(a1)
	move.b	#4,render_flags(a1)
	bset	#6,render_flags(a1)
