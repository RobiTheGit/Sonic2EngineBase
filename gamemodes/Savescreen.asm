...
loc_C912:
		...
		add.w	d0,d0
		moveq	#0,d1
		move.b	Zone_Numbers(pc,d0.w),d1
		bpl.s	loc_C932
...
loc_C936:
		move.w	d1,(a6)
		moveq	#0,d1
		move.b	Zone_Numbers+1(pc,d0.w),d1
		addi.w	#make_art_tile($562,1,1),d1
		move.w	d1,(a6)
		bra.s	loc_C94C
; ---------------------------------------------------------------------------
...
loc_C97A:
		lea	SaveFile_LifeContinueIcon(pc),a2
...
loc_C99A:
		...
		tst.b	(a0)
		bpl.s	loc_C9CC
		lea	NewSave_LivesContinues(pc),a1
		move.w	d7,d0
		addq.w	#6,d0
		bsr.w	sub_C87E
		moveq	#2-1,d1
...
Pal_Save_ZoneCardA:		binclude "art/Save Menu/Palettes/Zone Card A.bin"
	even
;We can stop worrying about here
...
.level_icons:
		...
		move.w	$36(a0),d1
		cmp.b	$3A(a0),d1
		bne.s	.testforstartedgame
		move.b	$3B(a0),d0
		beq.s	.testforstartedgame
		addq.b	#1,$23(a0)
		cmpi.b	#1,d0
		beq.s	.testfornewsave
		addq.b	#1,$23(a0)
		cmpi.b	#2,d0
		beq.s	.testfornewsave
		addq.b	#2,$23(a0)
		bra.s	.testforstartedgame
; ---------------------------------------------------------------------------

.testfornewsave:
		cmpi.w	#1,$34(a0)
		bls.s	.testforstartedgame
		move.b	#$23,$23(a0)

.testforstartedgame:
		tst.w	$38(a0)
		bne.s	.testforcompletedgame
		tst.b	$3B(a0)
		beq.w	.testforplayer2
		st	$38(a0)

.testforcompletedgame:
		tst.w	(Player_2+object_control).w
		bne.s	.preparechildren
		tst.w	(Events_bg+$12).w
		beq.s	.set_completed_icons
		clr.b	$1D(a0)
		move.w	#2,$16(a0)
		bra.w	.preparechildren
; ---------------------------------------------------------------------------

.set_completed_icons:
		moveq	#$B,d6
		cmpi.w	#3,$34(a0)
		beq.s	.previous_level
		moveq	#$C,d6
		cmpi.w	#2,$34(a0)
		beq.s	.previous_level
		cmpi.b	#2,$3B(a0)
		blo.s	.previous_level
		moveq	#$D,d6
.previous_level:
		moveq	#0,d2
		move.w	$36(a0),d1
...
.loadlevel:
		...
		beq.s	.newsavelives
		cmpi.b	#3,d0
		bhs.s	.getlivesandcontinues
		tst.b	9(a1)
		bne.s	.getlivesandcontinues

.newsavelives:
		move.b	#3,8(a1)
		subq.b	#1,9(a1)
		bcc.s	.getlivesandcontinues
		clr.b	9(a1)

.getlivesandcontinues:
...
locret_D70A:
		rts
; End of function ChangePlayerMode
...
.setlevel:
		andi.w	#$F,d0
		add.w	d0,d0
		move.w	DataSelectLevelOrder(pc,d0.w),d0

.return:
		rts
; End of function SetLevelForSave
EHZ  =  0
ID1  =  $100
ID2  =  $200
ID3  =  $300
...
byte_DB31:
    ...
    charset
KosArt_To_VDP:
		movea.l	a1,a3		; a1 will be changed by Kos_Decomp_Old, so we're backing it up to a3
		jsr	(Kos_Decomp_Old).l
