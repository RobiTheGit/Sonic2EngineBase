...
Obj01_InWater:
	...
	movea.l	a0,a1
	bsr.w	ResumeMusic
	move.l	#Obj0A,(Sonic_BreathingBubbles).w ; load Obj0A (sonic's breathing bubbles) at $FFFFD080
	move.b	#$81,(Sonic_BreathingBubbles+subtype).w
	move.l	a0,(Sonic_BreathingBubbles+objoff_40).w
...
+
	...
	ble.s	Sonic_JumpMoves		; if not, branch
	move.b	(Ctrl_1_Held_Logical).w,d0
...
Sonic_JumpMoves:
	...
	move.b	(Ctrl_1_Press_Logical).w,d0
	andi.b	#button_B_mask,d0 ; is a jump button pressed?
	bne.s	Sonic_DoubleJump
	move.b	(Ctrl_1_Press_Logical).w,d0
	andi.b	#button_C_mask,d0 ; is a jump button pressed?
	bne.s	Sonic_Dash
	rts
Sonic_DashDown:
	move.w	#0,x_vel(a0)		; halt horizontal speed...
	move.w	#0,ground_vel(a0)	; ...both ground and air
	move.w	#$780,y_vel(a0)		; force Sonic down
	clr.b	jumping(a0)
	move.b	#1,double_jump_flag(a0)
...
Sonic_CheckGoSuper:
	...
	move.l	#Obj7E,(SuperSonicStars).w ; load Obj7E (super sonic stars object) at $FFFFD040
	move.w	#$A00,(Sonic_top_speed).w
	move.w	#$30,(Sonic_acceleration).w
	move.w	#$100,(Sonic_deceleration).w
...
SonAni_Bubble:	dc.b  $B,$5A,$5A,$11,$12,$FD,  0 ; breathe
	rev02even
SonAni_DeathBW:	dc.b $20,$D6,$D7,$FF
	rev02even
